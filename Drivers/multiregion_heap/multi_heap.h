...
/*
This example contains 6 regions (stm32f746xx for external 8MB sdram chip with FMC)
- 0: ITC internal 64kB      (0x00000000..0x00003FFF)
- 1: DTC internal 64kB     	(0x20000000..0x2000FFFF)
- 2: D1 internal 240kB ram  (0x20010000..0x2004BFFF, shared with stack and bss)
- 3: D2 internal 16kB ram  	(0x2004C000..0x2004FFFF)
- 4: external 8MB sdram     (0xC0000000..0xCFFFFFFF)
#define HEAP_NUM          5
#define HEAP_REGIONS      {{ (uint8_t *) 0x00000000, 0x4000       },\
						  { (uint8_t *) 0x20000000, 0x10000       },\
						  { (uint8_t *) &ucHeap0, sizeof(ucHeap0) },\
						  { (uint8_t *) 0x2004C000, 0x4000        },\
						  { (uint8_t *) 0xC0000000, 0x10000000      }};
//#define RTOSREGION        2                      // pvPortMalloc, vPortFree, xPortGetFreeHeapSize : region 2
...
#define MALLOC_EXTREGION  4, -1, -1, -1, -1  	// malloc_ext, calloc_ext, realloc_ext, heapsize_ext : region 5
*/
...
- step 3: check the free memory in shared region
